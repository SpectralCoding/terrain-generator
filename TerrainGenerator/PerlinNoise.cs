...
namespace TerrainGenerator {
	class PerlinNoise {
		...
		public PerlinNoise(int seed) {
			...
			int i;
			for (i = 0; i < 256; i++) {
				ByteList.Add(Convert.ToByte(i));
			}
			i = ByteList.Count;
			while (i > 1) {
				i--;
				//int k = MT19937.Random(i + 1);
				int k = Convert.ToInt32(SFMT.Random() * (i + 1));
				byte value = ByteList[k];
				ByteList[k] = ByteList[i];
				ByteList[i] = value;
			}
			for (i = 0; i < 256; i++) {
				_perm[i] = ByteList[i];
		...
		private void InitGradients() {
			for (int i = 0; i < GradientSizeTable; i++) {
				//double z = 1f - 2f * MT19937.Random();
				double z = 1f - 2f * SFMT.Random();
				double r = Math.Sqrt(1f - z * z);
				//double theta = 2 * Math.PI * MT19937.Random();
				double theta = 2 * Math.PI * SFMT.Random();
